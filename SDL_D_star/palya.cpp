...
inline bool operator<(const UtPos& lhs, const UtPos& rhs)
{
    ...
    Jarokelo::Jarokelo(float posx, float posy, float celx, float cely, vector<UtPos> utvn, int st_time){
        /*
        irany=0.f;
        sebesseg=0.f;
        fordulas=0;
...
Mezo::Mezo(int x, int y){reset(x,y);} // létrehozza a mezőt, default értékekkel nem túl hasznos

void Mezo::reset(int x, int y){       // a reset viszont hasznos
